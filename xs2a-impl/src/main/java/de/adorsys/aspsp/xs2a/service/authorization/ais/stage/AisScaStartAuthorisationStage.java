...
public class AisScaStartAuthorisationStage extends AisScaStage<UpdateConsentPsuDataReq, UpdateConsentPsuDataResponse> {
    ...
    private UpdateConsentPsuDataResponse createResponseForMultipleAvailableMethods(Xs2aPsuData psuData, List<SpiAuthenticationObject> availableScaMethods) {
        UpdateConsentPsuDataResponse response = new UpdateConsentPsuDataResponse();
        response.setPsuId(psuData.getPsuId());
        response.setAvailableScaMethods(availableScaMethods.stream().map(spiToXs2aAuthenticationObjectMapper::mapToXs2aAuthenticationObject).collect(Collectors.toList()));
    ...
    private UpdateConsentPsuDataResponse createResponseForOneAvailableMethod(SpiAccountConsent accountConsent, Xs2aPsuData psuData, SpiAuthenticationObject scaMethod, String consentId) {
