...
public class SpiPaymentFactory {
     ...
     *
     * @param pisPaymentList     PisPayment
     ...
     */
    public Optional<? extends SpiPayment> createSpiPaymentByPaymentType(List<PisPayment> pisPaymentList, String paymentProduct, PaymentType paymentType) {
        switch (paymentType) {
            case SINGLE:
                return createSpiSinglePayment(pisPaymentList.get(0), paymentProduct);
            case PERIODIC:
                return createSpiPeriodicPayment(pisPaymentList.get(0), paymentProduct);
            case BULK:
                return createSpiBulkPayment(pisPaymentList, paymentProduct);
            default:
     ...
     *
     * @param pisPaymentList List of PisPayments
     * @param paymentProduct PaymentProduct
     * @return Optional of SpiBulkPayment from PisPayment
     */
    public Optional<SpiBulkPayment> createSpiBulkPayment(List<PisPayment> pisPaymentList, String paymentProduct) {
        BulkPayment bulkPayment = cmsToXs2aPaymentMapper.mapToBulkPayment(pisPaymentList);
