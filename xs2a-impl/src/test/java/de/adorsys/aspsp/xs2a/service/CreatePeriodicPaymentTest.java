...
public class CreatePeriodicPaymentTest {
    ...
    public void success_initiate_periodic_payment() {
        //When
        when(scaPaymentService.createPeriodicPayment(buildPeriodicPayment(), PaymentProduct.SCT, buildXs2aPisConsent().getConsentId())).thenReturn(buildPeriodicPaymentInitiationResponse());
        ResponseObject<PeriodicPaymentInitiationResponse> actualResponse = createPeriodicPaymentService.createPayment(buildPeriodicPayment(), buildPaymentInitiationParameters(), TPP_INFO, buildXs2aPisConsent());
