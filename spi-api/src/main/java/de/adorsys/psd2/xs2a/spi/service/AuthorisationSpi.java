...
interface AuthorisationSpi<T> {
     ...
     *
     * @param psuData              ASPSP identifier(s) of the psu
     * @param authenticationObject Chosen sca method
     * @param businessObject       generic payment object
     * @param aspspConsentData     Encrypted data that may stored in the consent management system in the consent linked to a request.
     *                             May be null if consent does not contain such data, or request isn't done from a workflow with a consent
     ...
     */
    @NotNull
    SpiResponse<SpiAuthorizationCodeResult> requestAuthorisationCode(@NotNull SpiPsuData psuData, @NotNull SpiAuthenticationObject authenticationObject, @NotNull T businessObject, @NotNull AspspConsentData aspspConsentData);
}
