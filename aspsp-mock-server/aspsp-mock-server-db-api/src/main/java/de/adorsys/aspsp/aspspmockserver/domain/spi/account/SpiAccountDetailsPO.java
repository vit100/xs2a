...
@AllArgsConstructor
@NoArgsConstructor
public class SpiAccountDetailsPO {
    @Setter
    @NonFinal
    ...
    private SpiAccountStatusPO spiAccountStatus;
    ...
    private List<SpiAccountBalancePO> balances;

    public Optional<SpiAccountBalancePO> getFirstBalance() {
        return balances.stream()
                   .filter(bal -> SpiBalanceTypePO.INTERIM_AVAILABLE == bal.getSpiBalanceType())
                   .findFirst();
    }

    public void updateFirstBalance(SpiAccountBalancePO balance) {
        balances.stream()
            .filter(bal -> SpiBalanceTypePO.INTERIM_AVAILABLE == bal.getSpiBalanceType())
            .findFirst()
            .map(b -> balance);
